...
def landing(request):
    print(request)
    return render(request,'medic/home.html')
...
def createWebString(request):
   return render(request,'medic/create.html')

def webSites(request,cName):
    html_content=webSite.objects.get(companyName=cName).htmlString
    #print(html_content)
    return render(request,'medic/view.html',{'container':html_content,'companyName':cName})